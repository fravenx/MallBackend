...
//        System.out.println(flag);
    ...
    public ResponseData<?> setPassword(@RequestBody Password password) {
        Boolean flag = userService.setPassword(password);
        if(flag){
            return ResponseDataUtils.buildSuccess("0", "密码修改成功！");
        }
        return ResponseDataUtils.buildError("密码错误，请重新输入！");
    }
    ...
    public ResponseData<?> forgetPassword(@RequestBody Password password) {
        User user = userService.getUserByUsername(password.getUsername());

        if (user == null) {
            return ResponseDataUtils.buildError("用户名不存在，请重新输入！");
        }
